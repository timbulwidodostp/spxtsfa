...
syntax varlist,Uhet(varlist) [INItial(name) NOCONstant mldisplay(string) wxvars(varlist) ///
...
// 检查权重矩阵与数据是否匹配
	...
	local id=r(ivar)
	local time=r(tvar)
    qui keep `varlist'  `id' `time' `uhet' `touse' `wxvars'
    tempvar order0
...
//mata mata describe
    ...
    ereturn clear
	ml model d0 spxtsfauv0() (frontier:`yvar' = `xvars' `wxvars2',`noconstant')(uhet: `uhet',noconstant) ///
  ...
  ereturn local cmdline `cmdline'
   ...
   if(`"`te'"'!=""){
		...
		mata:_te_order=spxtsfauv0_te(_b_ml,`nx',`nz',"`yvar'","`xvars' `wxvars2'","`uhet'","`noconstant'")
   }	
